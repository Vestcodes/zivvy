# Zivvy Sales — Order Submitted → Fulfillment
...
6. **Optional enrich:** Webhooks by Zapier GET `https://api.zivvy.xyz/v1/sales-orders/{{name}}` with Bearer `zk_live_`
...
Import/push `zapier-templates/integration` (Platform CLI) for a first-class **Sales Order Submitted** trigger — see root README.
