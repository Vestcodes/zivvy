...
const EVENTS = ["stock-entries.submitted","items.created","delivery-notes.submitted"];
...
const subscribeHook = async (z, bundle) => {
  const response = await z.request({
    ...
    body: {
      ...
      events: EVENTS,
      label: `Zapier: Stock Movement or Item Created`,
...
const performList = async (z, bundle) => {
  ...
  const response = await z.request({
    url: 'https://integrate.zivvy.xyz/v1/stock-entries',
    method: 'GET',
    params: { limit: 3 },
  });
...
module.exports = {
  ...
  noun: 'Stock Event',
  display: {
    label: 'Stock Movement or Item Created',
    description: 'Triggers on stock entry / delivery note submit or item create.',
  },
  operation: {
    type: 'hook',
    inputFields: [
