...
const EVENTS = ["leave-applications.created","leave-applications.updated","leave-applications.submitted","expense-claims.created","expense-claims.updated","expense-claims.submitted","employees.created"];
...
const subscribeHook = async (z, bundle) => {
  const response = await z.request({
    ...
    body: {
      ...
      events: EVENTS,
      label: `Zapier: Leave, Expense, or Employee Event`,
...
const performList = async (z, bundle) => {
  ...
  const response = await z.request({
    url: 'https://integrate.zivvy.xyz/v1/leave-applications',
    method: 'GET',
    params: { limit: 3 },
  });
...
module.exports = {
  ...
  noun: 'HR Event',
  display: {
    label: 'Leave, Expense, or Employee Event',
    description: 'Triggers on leave/expense create|update|submit or employee create. Filter by status in Zap.',
  },
  operation: {
    type: 'hook',
    inputFields: [
