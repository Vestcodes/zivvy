...
export const Tabs = ({
  ...
  const initial =
    (value ? propTabs.find((t) => t.value === value) : undefined) ?? propTabs[0];
  const [active, setActive] = useState<Tab>(initial);
  const [tabs, setTabs] = useState<Tab[]>(() => {
    if (!value) return propTabs;
    const idx = propTabs.findIndex((t) => t.value === value);
    if (idx <= 0) return propTabs;
    const newTabs = [...propTabs];
    const [sel] = newTabs.splice(idx, 1);
    newTabs.unshift(sel);
    return newTabs;
  });
