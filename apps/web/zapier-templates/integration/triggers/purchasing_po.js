...
const EVENTS = ["purchase-orders.submitted","purchase-receipts.submitted"];
...
const subscribeHook = async (z, bundle) => {
  const response = await z.request({
    ...
    body: {
      ...
      events: EVENTS,
      label: `Zapier: Purchase Order or Receipt Submitted`,
...
const performList = async (z, bundle) => {
  ...
  const response = await z.request({
    url: 'https://integrate.zivvy.xyz/v1/purchase-orders',
    method: 'GET',
    params: { limit: 3 },
  });
...
module.exports = {
  ...
  noun: 'Purchase Event',
  display: {
    label: 'Purchase Order or Receipt Submitted',
    description: 'Triggers when a PO or purchase receipt is submitted.',
  },
  operation: {
    type: 'hook',
    inputFields: [
