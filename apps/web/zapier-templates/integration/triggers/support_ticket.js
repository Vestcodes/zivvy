...
const EVENTS = ["support-tickets.created","support-tickets.updated","issues.created","issues.updated"];
...
const subscribeHook = async (z, bundle) => {
  const response = await z.request({
    ...
    body: {
      ...
      events: EVENTS,
      label: `Zapier: Support Ticket or Issue Event`,
...
const performList = async (z, bundle) => {
  ...
  const response = await z.request({
    url: 'https://integrate.zivvy.xyz/v1/support-tickets',
    method: 'GET',
    params: { limit: 3 },
  });
...
module.exports = {
  ...
  noun: 'Support Ticket',
  display: {
    label: 'Support Ticket or Issue Event',
    description: 'Triggers when a helpdesk ticket or issue is created/updated.',
  },
  operation: {
    type: 'hook',
    inputFields: [
