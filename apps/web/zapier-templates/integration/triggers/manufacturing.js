...
const EVENTS = ["work-orders.submitted","work-orders.updated","boms.created","job-cards.updated"];
...
const subscribeHook = async (z, bundle) => {
  const response = await z.request({
    ...
    body: {
      ...
      events: EVENTS,
      label: `Zapier: Work Order / BOM / Job Card Event`,
...
const performList = async (z, bundle) => {
  ...
  const response = await z.request({
    url: 'https://integrate.zivvy.xyz/v1/work-orders',
    method: 'GET',
    params: { limit: 3 },
  });
...
module.exports = {
  ...
  display: {
    label: 'Work Order / BOM / Job Card Event',
    description: 'Triggers on work order submit/update, BOM create, or job card update.',
  },
  operation: {
    type: 'hook',
    inputFields: [
