...
const EVENTS = ["bank-transactions.created","bank-transactions.updated"];
...
const subscribeHook = async (z, bundle) => {
  const response = await z.request({
    ...
    body: {
      ...
      events: EVENTS,
      label: `Zapier: New or Updated Bank Transaction`,
...
const performList = async (z, bundle) => {
  ...
  const response = await z.request({
    url: 'https://integrate.zivvy.xyz/v1/bank-transactions',
    method: 'GET',
    params: { limit: 3 },
  });
...
module.exports = {
  ...
  display: {
    label: 'New or Updated Bank Transaction',
    description: 'Triggers when a bank transaction is created or updated.',
  },
  operation: {
    type: 'hook',
    inputFields: [
